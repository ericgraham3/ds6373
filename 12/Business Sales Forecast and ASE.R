...
# ASE for model with lag and trend (last 5)
t=1:100
BSales$t = t
BSales2 = BSales[2:95,]
...
ad_tvFORECAST = fore.arma.wge(BSales$ad_tv,phi = est_ad_tv$phi, theta = est_ad_tv$theta, n.ahead = 5)
...
ad_onlineFORECAST = fore.arma.wge(BSales$ad_online,phi = est_online$phi, n.ahead = 5)
...
preds = predict(fit, newxreg = cbind(c(BSales$ad_tv[96],ad_tvFORECAST1$f[1:4]),c(BSales$ad_online[96],ad_onlineFORECAST1$f[1:4]),BSales$t[96:100]))
...
#Model 6 ... show the type = "both" is the same as adding a time as an exogenous variable
